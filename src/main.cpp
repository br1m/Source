...
unsigned int nTargetSpacing = 1 * 120; // 30 secs
...
const string strMessageMagic = "br1m Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight+1 >= 1 && pindexBest->nHeight+1 <= 1)
  {
    ...
    else if(pindexBest->nHeight+1 >= 2 && pindexBest->nHeight+1 <= 300002)
  {
    nSubsidy = 7 * COIN;
  }
    else if(pindexBest->nHeight+1 >= 300003 && pindexBest->nHeight+1 <= 800002)
  {
    nSubsidy = 3.5 * COIN;
  }
    else if(pindexBest->nHeight+1 >= 800003 && pindexBest->nHeight+1 <= 2400002)
  {
    nSubsidy = 1.75 * COIN;
  }
...
// br1m: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "br1m", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Gunman kills at least 25 in Texas Church";
        CTransaction txNew;
        txNew.nTime = 1509879213;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xcdbd5544fc95308e169581e9654a45809641bf46433774119e5a89873519c07d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb2, 0xc5, 0xf6 };
